...
class CatDogClassifierFromScratch:
    """Neural Network implementation from scratch matching NNTrain.py architecture"""
    
    def __init__(self, input_shape=(224, 224, 1)):
        self.input_shape = input_shape
        self.input_size = np.prod(input_shape)  # 224 * 224 * 1 = 50176
        
        # Initialize layers matching the architecture
        self.layers = []
        self.batch_norms = []
        self.dropouts = []
        
        # Layer 1: Dense(2048) + BatchNorm + Dropout(0.5)
        self.layers.append(DenseLayer(self.input_size, 2048))
        self.batch_norms.append(BatchNormalization(2048))
        self.dropouts.append(Dropout(0.5))
        
        # Layer 2: Dense(512) + BatchNorm + Dropout(0.4)
        self.layers.append(DenseLayer(2048, 512))
        self.batch_norms.append(BatchNormalization(512))
        self.dropouts.append(Dropout(0.4))
        
        # Layer 3: Dense(128) + BatchNorm + Dropout(0.3)
        self.layers.append(DenseLayer(512, 128))
        self.batch_norms.append(BatchNormalization(128))
        self.dropouts.append(Dropout(0.3))
        
        # Layer 4: Dense(32) + Dropout(0.2)
        self.layers.append(DenseLayer(128, 32))
        self.dropouts.append(Dropout(0.2))
        
        # Output layer: Dense(1) + Sigmoid
        self.layers.append(DenseLayer(32, 1))
    ...
    def forward(self, x, training=True):
        ...
        # Layer 1: Dense(2048) + ReLU + BatchNorm + Dropout
        ...
        # Layer 2: Dense(512) + ReLU + BatchNorm + Dropout
        x = self.layers[1].forward(x)
        x = ActivationFunctions.relu(x)
        x = self.batch_norms[1].forward(x, training)
        x = self.dropouts[1].forward(x, training)
        
        # Layer 3: Dense(128) + ReLU + BatchNorm + Dropout
        x = self.layers[2].forward(x)
        x = ActivationFunctions.relu(x)
        x = self.batch_norms[2].forward(x, training)
        x = self.dropouts[2].forward(x, training)
        
        # Layer 4: Dense(32) + ReLU + Dropout
        x = self.layers[3].forward(x)
        x = ActivationFunctions.relu(x)
        x = self.dropouts[3].forward(x, training)
    ...
    def backward(self, x, y_true, y_pred):
        ...
        # For sigmoid + binary cross-entropy, the gradient simplifies to (y_pred - y_true)
        grad = (y_pred - y_true) / batch_size
        
        # Store activations for ReLU derivatives
        activations = []
        
        # We need to store the pre-activation values during forward pass
        # For now, let's recalculate them or use a simpler approach
        
        # Output layer backward (sigmoid is already included in loss gradient)
        grad, grad_weights_4, grad_bias_4 = self.layers[4].backward(grad)
        
        # Layer 4 backward: Dropout + ReLU
        grad = self.dropouts[3].backward(grad)
        # For ReLU derivative, we need the pre-activation values
        # Let's compute them from the stored input and weights
        pre_activation_3 = np.dot(self.layers[3].input, self.layers[3].weights) + self.layers[3].bias
        relu_grad_3 = (pre_activation_3 > 0).astype(float)
        grad = grad * relu_grad_3
        grad, grad_weights_3, grad_bias_3 = self.layers[3].backward(grad)
        
        # Layer 3 backward: Dropout + BatchNorm + ReLU
        grad = self.dropouts[2].backward(grad)
        grad, grad_gamma_2, grad_beta_2 = self.batch_norms[2].backward(grad)
        # ReLU derivative
        pre_activation_2 = np.dot(self.layers[2].input, self.layers[2].weights) + self.layers[2].bias
        relu_grad_2 = (pre_activation_2 > 0).astype(float)
        grad = grad * relu_grad_2
        grad, grad_weights_2, grad_bias_2 = self.layers[2].backward(grad)
        
        # Layer 2 backward: Dropout + BatchNorm + ReLU
        grad = self.dropouts[1].backward(grad)
        grad, grad_gamma_1, grad_beta_1 = self.batch_norms[1].backward(grad)
        # ReLU derivative
        ...
        # Layer 1 backward: Dropout + BatchNorm + ReLU
        grad = self.dropouts[0].backward(grad)
        grad, grad_gamma_0, grad_beta_0 = self.batch_norms[0].backward(grad)
        # ReLU derivative
        ...
        return [
            (grad_weights_0, grad_bias_0, grad_gamma_0, grad_beta_0),
            (grad_weights_1, grad_bias_1, grad_gamma_1, grad_beta_1),
            (grad_weights_2, grad_bias_2, grad_gamma_2, grad_beta_2),
            (grad_weights_3, grad_bias_3, None, None),
            (grad_weights_4, grad_bias_4, None, None)
        ]
    ...
    def update_parameters(self, gradients):
        ...
        for i, (grad_weights, grad_bias, grad_gamma, grad_beta) in enumerate(gradients):
            # Update dense layer parameters
            layer = self.layers[i]
            
            # Adam optimizer for weights
            if not hasattr(layer, 'weights_m'):
                layer.weights_m = np.zeros_like(layer.weights)
                layer.weights_v = np.zeros_like(layer.weights)
                layer.bias_m = np.zeros_like(layer.bias)
                layer.bias_v = np.zeros_like(layer.bias)
            
            # Update weights
            layer.weights_m = self.beta1 * layer.weights_m + (1 - self.beta1) * grad_weights
            layer.weights_v = self.beta2 * layer.weights_v + (1 - self.beta2) * (grad_weights ** 2)
            ...
            # Update bias
            layer.bias_m = self.beta1 * layer.bias_m + (1 - self.beta1) * grad_bias
            layer.bias_v = self.beta2 * layer.bias_v + (1 - self.beta2) * (grad_bias ** 2)
            
            bias_m_corrected = layer.bias_m / (1 - self.beta1 ** self.t)
            bias_v_corrected = layer.bias_v / (1 - self.beta2 ** self.t)
            ...
            if grad_gamma is not None and i < len(self.batch_norms):
                bn = self.batch_norms[i]
                
                if not hasattr(bn, 'gamma_m'):
                    bn.gamma_m = np.zeros_like(bn.gamma)
                    bn.gamma_v = np.zeros_like(bn.gamma)
                    bn.beta_m = np.zeros_like(bn.beta)
                    bn.beta_v = np.zeros_like(bn.beta)
                
                # Update gamma
                bn.gamma_m = self.beta1 * bn.gamma_m + (1 - self.beta1) * grad_gamma
                bn.gamma_v = self.beta2 * bn.gamma_v + (1 - self.beta2) * (grad_gamma ** 2)
                
                gamma_m_corrected = bn.gamma_m / (1 - self.beta1 ** self.t)
                gamma_v_corrected = bn.gamma_v / (1 - self.beta2 ** self.t)
                
                bn.gamma -= self.learning_rate * gamma_m_corrected / (np.sqrt(gamma_v_corrected) + self.epsilon)
                
                # Update beta
                bn.beta_m = self.beta1 * bn.beta_m + (1 - self.beta1) * grad_beta
                bn.beta_v = self.beta2 * bn.beta_v + (1 - self.beta2) * (grad_beta ** 2)
                
                beta_m_corrected = bn.beta_m / (1 - self.beta1 ** self.t)
                beta_v_corrected = bn.beta_v / (1 - self.beta2 ** self.t)
                
                bn.beta -= self.learning_rate * beta_m_corrected / (np.sqrt(beta_v_corrected) + self.epsilon)
    ...
    def train(self, train_generator, val_generator, epochs=100):
        ...
        print(f"Architecture: {self.input_size} -> 2048 -> 512 -> 128 -> 32 -> 1")
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(len(train_generator)):
                ...
                # Clean logging - either show detailed stats or progress bar, not both
                if batch_count % 10 == 0 or batch_count == len(train_generator):
                    # Clear any existing progress line
                    print(f"\r{' ' * 100}\r", end="")
                    
                    avg_loss = np.mean(train_losses[-10:])  # Average of last 10 batches
                    avg_acc = np.mean(train_accuracies[-10:])
                    ...
                    # Show simple progress bar for intermediate batches
                    ...
                    print(f"\r  [{progress_bar}] {progress_percent:5.1f}% - "
                          f"Loss: {loss:.4f}, Acc: {accuracy:.4f}", end="", flush=True)
            ...
            for batch_idx in range(len(val_generator)):
                ...
                # Show clean validation progress
                val_progress = ((batch_idx + 1) / len(val_generator)) * 100
                val_progress_bars = int(val_progress / 5)
                val_progress_bar = "=" * val_progress_bars + "-" * (20 - val_progress_bars)
                print(f"\r  [{val_progress_bar}] {val_progress:5.1f}% - "
                      f"Val Loss: {loss:.4f}, Val Acc: {accuracy:.4f}", end="", flush=True)
    ...
    def load_model(self, filename):
        ...
        self.batch_norms = model_data['batch_norms']
...
def main():
    ...
    for bn in classifier.batch_norms:
